...
public class WindDial extends JPanel implements WindDataListener {
  ...
  private Color rose_char_col = Color.BLACK;
  private Color needle_faint_col = Color.lightGray;
  private Color groove_col_low = Color.DARK_GRAY;
  private Color groove_col_high = Color.LIGHT_GRAY;
  ...
  public void justPaint(Graphics g)
  {
    ...
    if ( wind_angle >= 0.0 )
    {
      ...
      Shape dir_needle = at.createTransformedShape(new Polygon(xd_points, yd_points, 5));
      Shape dir_needle_l = at.createTransformedShape(new Polygon(xd_points_l, yd_points_l, 4));
      Shape dir_needle_r = at.createTransformedShape(new Polygon(xd_points_r, yd_points_r, 4));
  ...
  private void prepareWindDirectionDial(Graphics2D g2)
  {
    ...
    d_dial_g.drawImage(d_dial_image, 
        centre.x - d_rose_goove_outer,
        centre.y - d_rose_goove_outer,
        d_rose_goove_outer * 2,
        d_rose_goove_outer * 2,
        this);
    ...
    for ( int i = 0; i < rose_points.length; i++ )
    {
      ...
      boolean haveText = rose_points[i] != null && !rose_points[i].isEmpty();
    ...
    // Draw inner wind direction 'groove' above which arrow moves
    d_dial_g.setStroke(new BasicStroke(d_groove_line));
    d_dial_g.setColor(groove_col_low);
    d_dial_g.drawOval(centre.x - d_rose_goove,
        centre.y - d_rose_goove,
        d_rose_goove * 2,
        d_rose_goove * 2);
    d_dial_g.setColor(groove_col_high);
    d_dial_g.drawOval(centre.x - (d_rose_goove + (int) d_groove_line),
        centre.y - (d_rose_goove + (int) d_groove_line),
        (d_rose_goove  + (int) d_groove_line) * 2,
        (d_rose_goove + (int) d_groove_line) * 2);

    // Draw outer wind direction 'groove' above which arrow moves
    d_dial_g.setStroke(new BasicStroke(d_groove_line));
    d_dial_g.setColor(groove_col_low);
    d_dial_g.drawOval(centre.x - d_rose_goove_outer,
        centre.y - d_rose_goove_outer,
        d_rose_goove_outer * 2,
        d_rose_goove_outer * 2);
    d_dial_g.setColor(groove_col_high);
    d_dial_g.drawOval(centre.x - (d_rose_goove_outer + (int) d_groove_line),
        centre.y - (d_rose_goove_outer + (int) d_groove_line),
        (d_rose_goove_outer  + (int) d_groove_line) * 2,
        (d_rose_goove_outer + (int) d_groove_line) * 2);
